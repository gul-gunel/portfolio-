...
// navigation  --  blog inner
let tabs = document.querySelectorAll(".nav__link");
let contents = document.querySelectorAll(".right-content");


let links = document.querySelectorAll(".blog__content__right");
let blogInners = document.querySelectorAll(".blog-inner");
...
tabs.forEach((tab, index) => {
	tab.addEventListener("click", () => {
		...
		// blog-inner closed
		blogInners.forEach((blogInner) => {
			blogInner.classList.remove("active");
		})
	});
});
...
links.forEach((link, i) => {
	link.addEventListener('click', () => {
		...
		blogInners[i].classList.add("active");
	})
})
